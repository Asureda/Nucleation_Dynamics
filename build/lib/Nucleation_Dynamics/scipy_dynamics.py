...
class ScipyClusterDynamics:
    ...
    def dy_dt_open(self, t, y):
        ...
        changes[-1] = -self.backward_rate_array[-1] * y[-1] + \
                      self.forward_rate_array[-2] * y[-2] - \
                      self.forward_rate_array[-1] * self.y[-1]
        return changes
    ...
    def simulate(self, method='RK45', t_span=None, y0=None, t_eval=None, rtol=1e-3, atol=1e-6, **kwargs):
            ...
            # Seleccionar la función dy_dt correcta según el tipo de límite
            dy_dt_func = self.dy_dt_closed if self.boundary_type == 'closed' else self.dy_dt_open

            # Llamar a solve_ivp
            sol = solve_ivp(fun=dy_dt_func, t_span=t_span, y0=y0, method=method, t_eval=t_eval, rtol=rtol, atol=atol, vectorized=False, **kwargs)
            ...
            if sol.success:
                ...
                # Calcular dy/dt para cada t_eval
                for i, t in enumerate(sol.t):
                    self.dydt_array[:, i] = dy_dt_func(t, sol.y[:, i])
            else:
