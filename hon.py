...
class HON:
    ...
    def __init__(self, input, c_type, time):
        ...
        self.surface_tension = ureg.Quantity(self.input["surface_tension"], "N/m")
        self.solubility = ureg.Quantity(self.input["solubility"], "mol/m**3/Pa")
    ...
    def S0_HON(self, T, C, M):
        N_A =  6.0221409e+23*ureg.Quantity(1.0,"1/mol")
        C_ = C*ureg.Quantity(1.0,"mol/m**3")
        ...
        if self.supersaturation_ratio > 0:
            ...
            S0 = (((self.atom_volume * N_A**2 * C_**2) / self.supersaturation_ratio) * A).to_base_units()
        else:
    ...
    def saturation_driving_force(self, T):
        ...
        Kb_T = k_b*T_k
        if self.supersaturation_ratio > 0:
            Dg = ((-Kb_T * math.log(self.supersaturation_ratio)) / (self.atom_volume)).to('joule/m**3')
        else:
    ...
    def critical_DG(self, T):
        if self.Dgvol != 0:
            DG = ((16.0 * math.pi * self.surface_tension**3) / (3.0 * self.Dgvol**2)).to('joule')
        else:
            DG = ureg.Quantity(0.0, "joule")
        return DG
    def critical_radius(self):
        if self.Dgvol != 0:
            r = ((-2.0 * self.surface_tension) / (self.Dgvol)).to('m')
        else:
    ...
    def S_HON(self, T, C, C_sat, M):
        self.supersaturation_ratio = C / C_sat
